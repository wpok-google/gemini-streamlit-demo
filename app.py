...
with tab2:
  ...
  if generate_t2t and prompt:
    ...
    with st.spinner(
        "Generating your marketing campaign using Gemini 1.0 Pro ..."
    ):
      with second_tab1:
        response = get_gemini_pro_text_response(
            text_model_pro,
...
with tab3:
  ...
  image_undst, screens_undst, diagrams_undst, recommendations, sim_diff = (
      st.tabs([
  ...
  with image_undst:
    ...
    st.write(
        "Our expectation: Recommend a chair that would complement the given"
    ...
    content = [
        ...
        chair_4_image,
        (
            "\nFor each chair, explain why it would be suitable or not suitable"
            " for the following room:"
        ),
        room_image,
        (
            "Only recommend for the room provided and not other rooms. Provide"
            " your recommendation in a table format with chair name and reason"
            " as columns."
        ),
    ]
  ...
  with screens_undst:
    ...
    st.write(
        "Equipped with the ability to extract information from visual elements"
        " on screens, Gemini 1.0 Pro Vision can analyze screenshots, icons, and"
        " layouts to provide a holistic understanding of the depicted scene."
    )
    ...
    st.write(
        "Our expectation: Provide instructions for resetting the clock on this"
        " appliance in English"
    )
...
"""
  ...
  with diagrams_undst:
    ...
    st.write(
        "Gemini 1.0 Pro Vision multimodal capabilities empower it to comprehend"
        " diagrams and take actionable steps, such as optimization or code"
        " generation. The following example demonstrates how Gemini 1.0 can"
        " decipher an Entity Relationship (ER) diagram."
    )
    er_diag_img = Part.from_uri(er_diag_uri, mime_type="image/jpeg")
    st.image(er_diag_url, width=350, caption="Image of a ER diagram")
    st.write(
        "Our expectation: Document the entities and relationships in this ER"
        " diagram."
    )
  ...
  with recommendations:
    ...
    st.write(
        "Our expectation: Suggest which glasses type is better for the"
        f" {face_type} face shape"
    )
    content = [
  ...
  with sim_diff:
    ...
    st.write(
        "Gemini 1.0 Pro Vision can also recognize math formulas and equations"
        " and extract specific information from them. This capability is"
        " particularly useful for generating explanations for math problems, as"
        " shown below."
    )
...
Provide the answer in table format.
  ...
  with video_geolocation:
    ...
    video_geolocation_url = (
        "https://storage.googleapis.com/"
        + video_geolocation_uri.split("gs://")[1]
    )
